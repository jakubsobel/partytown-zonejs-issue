...
(self => {
    ...
    } ] : (added = added || new Set) && Array.isArray(value) ? added.has(value) ? [ 1, [] ] : added.add(value) && [ 1, value.map((v => serializeForMain($winId$, $instanceId$, v, added))) ] : "object" === type ? value[InstanceIdKey] ? [ 3, {
    ...
    const addStorageApi = (win, storageName, storages, isSameOrigin) => {
        let getItems = items => {
        ...
        let storage = {
            ...
            setItem(key, value) {
                ...
                isSameOrigin && callMethod(win, [ storageName, "setItem" ], [ key, value ], 2);
            },
            removeItem(key) {
                index = getIndexByKey(key);
                index > -1 && getItems().splice(index, 1);
                isSameOrigin && callMethod(win, [ storageName, "removeItem" ], [ key ], 2);
            },
            key(index) {
                item = getItems()[index];
            ...
            clear() {
                getItems().length = 0;
                isSameOrigin && callMethod(win, [ storageName, "clear" ], EMPTY_ARRAY, 2);
            },
            get length() {
    ...
    const getPartytownScript = () => `<script src="${partytownLibUrl("partytown.js?v=0.4.2")}"><\/script>`;
    ...
    const patchHTMLScriptElement = WorkerHTMLScriptElement => {
    ...
    const createNodeCstr = (win, WorkerBase) => {
    ...
    const patchDocument = (WorkerDocument, isSameOrigin) => {
        const DocumentDescriptorMap = {
            ...
            currentScript: {
                get() {
                    ...
                    return currentScriptId ? getOrCreateNodeInstance(winId, currentScriptId, "SCRIPT") : null;
                }
            },
            defaultView: {
                get() {
    ...
    const patchDocumentElementChild = WokerDocumentElementChild => {
    ...
    const patchHTMLHtmlElement = WorkerHTMLHtmlElement => {
    ...
    const patchElement = WorkerElement => {
    ...
    const patchHTMLAnchorElement = WorkerHTMLAnchorElement => {
    ...
    const patchHTMLIFrameElement = WorkerHTMLIFrameElement => {
    ...
    const createWindow = ($winId$, $parentWinId$, url, isIframeWindow) => {
        ...
        const WorkerWindow = defineConstructorName(class extends WorkerBase {
            constructor() {
                    ...
                    if (!hasInitializedMedia) {
                        (() => {
                            if (!webWorkerCtx.$initWindowMedia$) {
                                ...
                                webWorkerCtx.$importScripts$(partytownLibUrl("partytown-media.js?v=0.4.2"));
                                webWorkerCtx.$initWindowMedia$ = self.$bridgeFromMedia$;
                                delete self.$bridgeFromMedia$;
                            }
                            return webWorkerCtx.$initWindowMedia$;
                        })()(WorkerBase, WorkerEventTargetProxy, environments[$winId$], win, windowMediaConstructors);
                ...
                environments[$winId$] = {
                    ...
                    $document$: $createNode$("#document", "1"),
                    $documentElement$: $createNode$("HTML", "2"),
                    $head$: $createNode$("HEAD", "3"),
                    $body$: $createNode$("BODY", "4"),
                    $location$: $location$,
